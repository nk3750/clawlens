...
export function getEffectiveTier(
  entry: AuditEntry,
  evalIdx: Map<string, AuditEntry>,
): RiskTierLabel | undefined {
  if (entry.toolCallId) {
    const evalEntry = evalIdx.get(entry.toolCallId);
    if (evalEntry?.riskTier) return evalEntry.riskTier;
  }
  if (entry.riskTier) return entry.riskTier;
...
function isEntryAcked(store: AttentionStore | undefined, toolCallId: string | undefined): boolean {
  if (!store || !toolCallId) return false;
 ...
 */
export function deriveAttentionFlags(att: AttentionResponse): {
  agents: Set<string>;
  ...
  const set = (agentId: string, reason: string) => {
    agents.add(agentId);
    if (!reasons.has(agentId)) reasons.set(agentId, reason);
  };
 ...
 */
export function buildSplitSessionIndex(entries: AuditEntry[]): Map<string, string> {
