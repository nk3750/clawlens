 ...
 */
export const SUMMARY_LLM_DEGRADED_NO_KEY_MESSAGE =
 ...
 */
function llmDegradedNoKeySummary(sessionKey: string): SessionSummary {
  return {
    sessionKey,
...
/**
 * Internal discriminated union from generateLlmSummary. `no_key` is the only
 * caller-actionable failure today — it means modelAuth resolved cleanly but
 * had nothing to hand back, so the operator must wire a provider key into
 * OpenClaw. Anything else (no model config, modelAuth threw, upstream API
 * returned nothing) falls through as "other" so the caller can route to the
 * template fallback without losing the per-call no-key signal.
 */
type LlmSummaryResult =
  | { ok: true; summary: SessionSummary }
  | { ok: false; reason: "no_key" | "other" };
...
async function generateLlmSummary(
  ...
  if (!needsDirectApi) return { ok: false, reason: "other" };
  ...
  if (!text) return { ok: false, reason: "other" };
