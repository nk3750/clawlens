 ...
 */
function isSkippablePrefix(token) {
    // cd, source/., set, export, pushd, popd, eval, env (the command), sudo, nohup, time
    return /^(cd|source|\.|set|export|pushd|popd|eval|env|sudo|nohup|time|nice|ionice|command)$/.test(token);
}
...
// ── URL extraction ────────────────────────────────────────────
const URL_PATTERN = /(?:https?:\/\/[^\s'"]+|localhost(?::\d+)?(?:\/[^\s'"]*)?|127\.\d+\.\d+\.\d+(?::\d+)?(?:\/[^\s'"]*)?)/gi;
...
function isNetworkWrite(tokens) {
...
function classifyGit(tokens) {
    ...
    for (let i = 1; i < tokens.length; i++) {
        ...
        if (GIT_WRITE_SUBCOMMANDS.has(t))
            return "git-write";
        if (GIT_READ_SUBCOMMANDS.has(t))
            return "git-read";
...
function isPackageInstall(tokens) {
    // Check if any token (after the command name) is an install-type subcommand
    for (let i = 1; i < tokens.length; i++) {
        if (PACKAGE_INSTALL_SUBCOMMANDS.has(tokens[i]))
            return true;
    }
    return false;
}
// ── Persistence subcommand check ──────────────────────────────
function classifyPersistence(command, tokens) {
 ...
 */
function findPrimaryCommand(tokens) {
    let i = 0;
...
// ── Main parser ───────────────────────────────────────────────
export function parseExecCommand(rawCommand) {
...
function classifyCommand(command, tokens, _fullCommand) {
    // Normalize command name (handle python3.11 → python3 → python)
    const normalized = command.replace(/\d+(\.\d+)*$/, "");
    const commandLower = command.toLowerCase();
    const normalizedLower = normalized.toLowerCase();
    // Check each category
    if (ECHO_COMMANDS.has(commandLower))
        return "echo";
    if (READ_ONLY_COMMANDS.has(commandLower))
        return "read-only";
    if (SEARCH_COMMANDS.has(commandLower))
        return "search";
    if (SYSTEM_INFO_COMMANDS.has(commandLower))
        return "system-info";
    if (DESTRUCTIVE_COMMANDS.has(commandLower))
        return "destructive";
    if (PERMISSION_COMMANDS.has(commandLower))
        return "permissions";
    if (PERSISTENCE_COMMANDS.has(commandLower))
        return classifyPersistence(commandLower, tokens);
    if (REMOTE_COMMANDS.has(commandLower))
        return "remote";
...
//# sourceMappingURL=exec-parser.js.map