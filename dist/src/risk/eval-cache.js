...
export class EvalCache {
     ...
     */
    static buildKey(toolName, params) {
     ...
     */
    maybeCache(toolName, params, evaluation, llmEvalThreshold) {
        if (evaluation.confidence !== "high")
            return false;
        if (evaluation.adjustedScore >= llmEvalThreshold)
            return false;
        const key = EvalCache.buildKey(toolName, params);
        ...
        const tier = evaluation.adjustedScore >= 80
            ? "critical"
     ...
     */
    warmFromAuditLog(entries) {
        // Filter to entries with high-confidence LLM evaluations
        const qualifying = entries.filter((e) => e.llmEvaluation && e.llmEvaluation.confidence === "high");
