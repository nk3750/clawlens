...
function pathMatches(params, ...patterns) {
    const p = typeof params.path === "string"
        ? params.path
...
function allUrlsAreLocal(urls) {
    if (urls.length === 0)
        return true; // no URLs = no external network
    return urls.every(isLocalhostUrl);
}
// ── Exec-specific modifiers (use parsed command info) ────────
const EXEC_MODIFIERS = [
    ...
    // (not test/[/tar/tail/awk/ssh/grep where -f means something else)
    {
        match: (_t, _p, parsed) => {
            ...
            if (!FORCE_F_COMMANDS.has(parsed.primaryCommand))
                return false;
    ...
    // network: split into local vs external based on parsed URLs
    {
        match: (_t, _p, parsed) => {
            if (!parsed)
                return false;
            if (parsed.category !== "network-read" && parsed.category !== "network-write")
                return false;
...
export function computeRiskScore(toolName, params, llmEvalThreshold = 50) {
    const modifiers = [];
    const tags = [];
    let base;
    let parsed;
