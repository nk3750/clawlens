...
import { extractCommandForGuardrail, extractIdentityKey, extractPathsForGuardrail, extractUrlsForGuardrail, } from "./identity.js";
import { isValidGuardrail, } from "./types.js";
// ── Glob predicates ─────────────────────────────────────────
...
export class GuardrailStore {
    ...
    load() {
        ...
        if (!parsed ||
            ...
            console.warn(`[clawlens] guardrails: file ${this.filePath} missing 'guardrails' array; ignoring`);
            return;
        }
        const all = parsed.guardrails;
        const valid = [];
        let dropped = 0;
        ...
        if (dropped > 0) {
            console.warn(`[clawlens] guardrails: dropped ${dropped} invalid entr${dropped === 1 ? "y" : "ies"} from ${this.filePath}`);
        }
    ...
    save() {
        ...
        const data = { guardrails: this.rules.map((r) => r.rule) };
    ...
    match(agentId, toolName, params) {
        ...
        for (const indexed of this.rules) {
            if (!matchesSelector(indexed.rule.selector, agentId, toolName, params))
                continue;
            if (!matchesTarget(indexed, toolName, params, getIdentityKey))
                continue;
            return indexed.rule;
        }
        return null;
    ...
    findEquivalent(input) {
        for (const { rule } of this.rules) {
            if (selectorEquals(rule.selector, input.selector) &&
...
function matchesSelector(s, agentId, toolName, params) {
    if (s.agent !== null && s.agent !== agentId)
        return false;
    switch (s.tools.mode) {
        case "any":
            return true;
        ...
        case "category": {
            const execCategory = toolName === "exec" && typeof params.command === "string"
...
function matchesTarget(indexed, toolName, params, getIdentityKey) {
    ...
    switch (t.kind) {
        ...
        case "identity-glob": {
            ...
            if (indexed.literalIdentity)
                return key === t.pattern;
            return safeMinimatch(key, t.pattern);
        }
    }
}
function safeMinimatch(value, pattern) {
    try {
...
function selectorEquals(a, b) {
    if (a.agent !== b.agent)
        return false;
    if (a.tools.mode !== b.tools.mode)
        return false;
    switch (a.tools.mode) {
        case "any":
            return true;
        case "names": {
            if (b.tools.mode !== "names")
                return false;
