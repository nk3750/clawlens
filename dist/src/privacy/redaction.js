...
const SENSITIVE_QUERY_RE = /([?&])(token|api[-_]?key|access[-_]?token|refresh[-_]?token|client[-_]?secret|secret|password|passwd|key)=([^&#\s]+)/gi;
...
const AUTHORIZATION_HEADER_RE = /(Authorization:\s*)(?:(Bearer|Basic|Digest|Token|API-Key|Apikey)\s+)?(\S+)/gi;
...
const TOKEN_CLI_RE = /(--(?:token|api[-_]?key|secret|access[-_]?token|refresh[-_]?token|client[-_]?secret)(?:\s+|=))(\S+)/gi;
...
function matchSensitiveKey(key) {
...
function redactHeaders(input) {
    let out = input.replace(AUTHORIZATION_HEADER_RE, (_m, label, scheme) => {
        const schemePart = scheme ? `${scheme} ` : "";
        return `${label}${schemePart}${REDACTION_MARKERS.authorization}`;
    });
    out = out.replace(GENERIC_TOKEN_HEADER_RE, (_m, label) => `${label}${REDACTION_MARKERS.token}`);
    out = out.replace(COOKIE_HEADER_RE, (_m, label) => `${label}${REDACTION_MARKERS.cookie}`);
    return out;
}
function redactCliFlags(input) {
    let out = input.replace(PASSWORD_CLI_RE, (_m, prefix) => `${prefix}${REDACTION_MARKERS.password}`);
    out = out.replace(SHORT_PASSWORD_CLI_RE, (_m, prefix) => `${prefix}${REDACTION_MARKERS.password}`);
    out = out.replace(TOKEN_CLI_RE, (_m, prefix) => `${prefix}${REDACTION_MARKERS.token}`);
    return out;
}
function redactEnvAssignments(input) {
    let out = input.replace(ENV_PASSWORD_ASSIGN_RE, (_m, name) => `${name}=${REDACTION_MARKERS.password}`);
    out = out.replace(ENV_KEY_ASSIGN_RE, (_m, name) => `${name}=${REDACTION_MARKERS.token}`);
    return out;
}
function redactTokenPrefixes(input) {
    let out = input;
...
function redactUrlContent(input) {
    let out = input.replace(URL_USERINFO_RE, (_m, scheme) => `${scheme}${REDACTION_MARKERS.urlCredential}@`);
    out = out.replace(SENSITIVE_QUERY_RE, (_m, sep, name) => `${sep}${name}=${REDACTION_MARKERS.token}`);
    return out;
}
/**
