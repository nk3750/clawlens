 ...
 */
export function matchTool(toolName, pattern) {
 ...
 */
export function matchParams(params, patterns) {
    for (const [key, pattern] of Object.entries(patterns)) {
        const value = params[key];
        if (value === undefined || value === null)
            return false;
        if (!globMatch(String(value), pattern))
            return false;
    }
    return true;
}
 ...
 */
export function matchRule(toolName, params, match) {
    // Tool name match
    if (match.tool !== undefined) {
        if (!matchTool(toolName, match.tool))
            return false;
    }
    // Param pattern match
    if (match.params !== undefined) {
        if (!matchParams(params, match.params))
            return false;
    }
    return true;
}
