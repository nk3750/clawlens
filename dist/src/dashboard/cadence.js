 ...
 */
export function extractCronRunStarts(entries) {
    const byKey = new Map();
    ...
    for (const group of byKey.values()) {
        ...
        for (let i = 1; i < sorted.length; i++) {
            gaps.push(new Date(sorted[i].timestamp).getTime() - new Date(sorted[i - 1].timestamp).getTime());
        }
        const medianGap = gaps.length > 0 ? [...gaps].sort((a, b) => a - b)[Math.floor(gaps.length / 2)] : 0;
        const threshold = Math.min(MAX_RUN_BOUNDARY_MS, Math.max(MIN_RUN_BOUNDARY_MS, medianGap * RUN_BOUNDARY_FACTOR));
        runStarts.push(sorted[0].timestamp);
        for (let i = 1; i < sorted.length; i++) {
            if (gaps[i - 1] > threshold)
                runStarts.push(sorted[i].timestamp);
        }
    }
    return runStarts;
}
 ...
 */
export function deriveScheduleLabel(mode, recentCronStarts, explicitSchedule) {
    if (explicitSchedule)
        return explicitSchedule;
    if (mode !== "scheduled")
        return null;
    if (recentCronStarts.length < 2)
        return null;
