...
export function classifyError(errMsg) {
    ...
    if (/credit balance|billing/i.test(errMsg))
        return "billing";
    if (/rate.?limit|\b429\b/i.test(errMsg))
        return "rate_limit";
    if (/\b5\d\d\b/.test(errMsg))
        return "provider";
    return "other";
}
export class LlmHealthTracker {
     ...
     *   - otherwise → ok
     */
    computeStatus(recent, failures) {
        if (failures === 0)
            return "ok";
        if (recent.length >= 3) {
            const lastThree = recent.slice(-3);
            if (lastThree.every((a) => !a.ok))
                return "down";
        }
        if (failures >= 3 && failures > recent.length / 2)
            return "degraded";
        return "ok";
    }
