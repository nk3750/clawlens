...
**Complement, don't compete.** ClawLens layers user-facing guardrails on top of system-level security. We don't duplicate, replace, or bypass built-in functionality.
...
- **Our value is different** — built-in is a firewall (automated, binary allow/deny, operator-configured). ClawLens is parental controls (human-in-the-loop, nuanced, user-configured).
...
ClawLens registers `before_tool_call` at priority 100 (plugin hook layer). Built-in exec approvals and tool profiles run in the tool execution pipeline (after hooks). Both layers contribute independently.

```
            Built-in          ClawLens          Result
Action A:   ALLOW (profile)   ALLOW (policy)    Proceeds
Action B:   ALLOW (profile)   BLOCK (policy)    Blocked by ClawLens
...
- **Do say:** "ClawLens adds human-in-the-loop approval on top of OpenClaw's built-in security"
- **Don't say:** "ClawLens replaces OpenClaw's security"
- **Analogy:** "Built-in = firewall, ClawLens = parental controls"
